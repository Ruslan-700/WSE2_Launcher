...
bool Class_Engine::UpdateOptions()
{
	...
	std::ifstream File_rgl_config(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config.ini"));
	std::string Line;
...
bool Class_Engine::ApplyOptions()
{
	...
	std::ifstream File_rgl_config(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config.ini"));
	std::remove(std::string(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config_temp.ini")).c_str());
	std::ofstream OutputFile_rgl_config(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config_temp.ini"));
	if (File_rgl_config.is_open() && OutputFile_rgl_config) {
		...
		if (std::remove(std::string(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config.ini")).c_str()) != 0) goto LABEL_ERROR;
		if (std::rename(std::string(std::string(CurentUserPath) + std::string("/Documents/Mount&Blade Warband WSE2/rgl_config_temp.ini")).c_str(), std::string(std::string(CurentUserPath) + "/Documents/Mount&Blade Warband WSE2/rgl_config.ini").c_str()) != 0) goto LABEL_ERROR;
		return true;
	}
	else {
	LABEL_ERROR:
